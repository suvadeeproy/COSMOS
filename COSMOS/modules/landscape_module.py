...
def make_synth(n=2000, seed=0):
    ...
    # create robust binary label with quantiles to guarantee two classes
    q = np.quantile(chi, 0.5)
    y_cls = (chi > q).astype(int)
    if y_cls.sum() == 0 or y_cls.sum() == len(y_cls):
        q = np.quantile(chi, 0.45)  # shift threshold slightly if degenerate
        y_cls = (chi > q).astype(int)
...
def panel():
    ...
    # Classify sign(chi) with guaranteed two classes (stratified split)
