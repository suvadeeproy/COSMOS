...
def panel():
    st.markdown("#### Landscape — GBM, conformal intervals, permutation importance, partial dependence")
    cols = st.columns(4)
    with cols[0]:
    ...
    with c1:
        ...
        st.pyplot(fig)
    with c2:
        st.write(f"Classifier AUC for sign(χ): **{auc:.3f}**")
        st.write(f"Empirical coverage at 90% nominal: **{coverage:.3f}**" if valid.any() else "Coverage unavailable")
    ...
    with st.expander("Physics notes"):
        st.markdown(f"""
**Targets**  
\(h_{11}\) and \(h_{21}\) are treated as responses; \(\chi=2(h_{11}-h_{21})\) defines a sign label. The predictors summarize toy combinatorial/variance features.

**Uncertainty**  
Conformal prediction constructs intervals with finite‑sample coverage under exchangeability. The observed fraction inside the bands estimates the achieved coverage \(\approx {coverage:.3f}\).

**Interpretability**  
Permutation importance estimates reliance by measuring the score drop when a single feature is randomized.  
Partial dependence shows \(\mathbb{E}[h_{11}\mid X_j=x]\) under the model. These tools aid structure discovery without causal claims.

**Metrics**  
\(R^2\) summarizes fit quality; AUC measures separability of sign regimes in \(\chi\).
"""))
