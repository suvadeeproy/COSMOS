...
def partial_dependence_1d(model, X, j, grid=None):
    ...
    Xref = X.copy(); yh = []
    for v in grid:
        Xref[:,j] = v; yh.append(model.predict(Xref).mean())
    return grid, np.array(yh)

def panel():
    st.markdown("#### Landscape — GBM, conformal intervals, FI, partial dependence, math")
    cols = st.columns(4)
    with cols[0]:
    ...
    with c2:
        ...
        st.write(f"Empirical coverage @90% nominal: **{coverage:.3f}**" if valid.any() else "Coverage: N/A")

    st.markdown("##### Permutation feature importance (ΔR2)")
    imps = perm_importance(reg, Xte, Yr_te[:,0], scorer=r2_score)
    fig3, ax3 = plt.subplots(figsize=(4,3))
    ax3.bar(["fsum","fvar","fcod"], imps); ax3.set_ylabel("ΔR2 when permuted"); st.pyplot(fig3)
    ...
    fig4, ax4 = plt.subplots(figsize=(4,3)); ax4.plot(grid, yh, "-")
    ax4.set_xlabel(["fsum","fvar","fcod"][j]); ax4.set_ylabel("E[h11 | feature]"); st.pyplot(fig4)
    ...
    with st.expander("Math/physics notes (derivations & meaning)"):
        st.latex(r"\textbf{Split‑conformal interval:}\; \widehat{C}(x) = \big[\hat f(x) \pm q_{1-\alpha}(|y-\hat f(x)|\_{\rm cal})\big]\;\Rightarrow\; \mathbb{P}(Y\in\widehat{C}(X)) \ge 1-\alpha\,\text{ under exchangeability.}")
        st.markdown("- The quantile \(q_{1-\alpha}\) is computed on a calibration set; the coverage is finite‑sample and distribution‑free.")
        st.latex(r"\textbf{Permutation FI:}\; I_j = R(\hat f;X,y) - R(\hat f;X^{\pi_j},y)\;\,\text{(higher means the model relies more on feature \(j\)).}")
        st.latex(r"\textbf{Partial dependence:}\; \mathrm{PD}_j(v) = \mathbb{E}[\hat f(X)\,|\,X_j=v]\;\approx\; \frac{1}{N}\sum_{i=1}^N \hat f(x^{(i)}\_{-j}, v)")
        st.markdown("Caveat: FI/PD reflect model behavior, not causal effects; use to form hypotheses, not conclusions.")
