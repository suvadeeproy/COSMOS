...
def panel():
    ...
    cols = st.columns(4)
    with cols[0]: n = st.slider("n samples", 1000, 20000, 4000, 500)
    with cols[1]: noise = st.slider("noise level", 0.2, 3.0, 1.0, 0.1)
    with cols[2]: trees = st.slider("n_estimators", 100, 800, 300, 50)
    with cols[3]: depth = st.slider("max_depth", 2, 6, 3, 1)
    ...
    with c1:
        ...
        ax.set_xlabel("true h11"); ax.set_ylabel("pred h11"); ax.set_title(f"Regression (R2={r2:.3f})")
        st.pyplot(fig)
    with c2:
        st.write(f"AUC for sign(chi): **{auc:.3f}**")
        st.write(f"Coverage @90% nominal: **{coverage:.3f}**" if valid.any() else "Coverage: N/A")

    st.markdown("##### Permutation feature importance (ΔR2)")
    imps = perm_importance(reg, Xte, Yr_te[:,0], scorer=r2_score)
    fig3, ax3 = plt.subplots(figsize=(4,3))
    ax3.bar(["fsum","fvar","fcod"], imps); ax3.set_ylabel("ΔR2 when permuted")
    st.pyplot(fig3)
    ...
    with st.expander("Physics + math: how to read the plots"):
        st.markdown(rf"""
**Accuracy (R²)**  
\[
R^2 = 1-\frac{{\sum_i (y_i-\hat y_i)^2}}{{\sum_i (y_i-\bar y)^2}} \approx {r2:.3f}.
\]
...
\]
here \(\alpha=0.1\) and empirical coverage ≈ **{coverage:.3f}**.

**AUC**  
\(\mathrm{{AUC}}=\Pr(s(X^+)>s(X^-))\approx {auc:.3f}\).

**Permutation FI**  
\(\Delta S_j = S(X,y)-S(\pi_j(X),y)\). Larger \(\Delta S_j\) means model reliance on feature \(j\).

**Partial dependence**  
\(\mathrm{{PD}}_j(x)=\mathbb{{E}}[\hat f(X)\mid X_j=x]\approx N^{-1}\sum_i \hat f(x,X_{i,-j}).\)
""")
