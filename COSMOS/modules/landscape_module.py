...
def panel():
    st.markdown("#### Landscape — GBM with intervals, permutation FI, partial dependence")
    cols = st.columns(4)
    with cols[0]:
    ...
    # Empirical coverage where intervals exist
    ...
    with st.expander("Physics discussion (live)"):
        st.markdown(f"""
- Conformal prediction targets 90% coverage, observed **{coverage:.3f}** on test where intervals exist.
- Permutation FI ranks feature influence by delta-R2; partial dependence traces mean response to a single feature.
- Mirrors real landscape analysis: predict invariants (e.g., h11) with guaranteed uncertainty and interpretable levers.
""")
