...
def panel():
    ...
    reg.fit(Xtr, Yr_tr[:,0])  # predict h11
    ...
    st.markdown("##### Permutation feature importance (ΔR² when feature shuffled)")
    imps = perm_importance(reg, Xte, Yr_te[:,0], scorer=r2_score)
    fig3, ax3 = plt.subplots(figsize=(4,3))
    ax3.bar(["fsum","fvar","fcod"], imps)
    ax3.set_ylabel("ΔR2"); st.pyplot(fig3)
    ...
    # Physics explainer & telemetry
    with st.expander("Physics/ML discussion • What changed with your controls?"):
        st.markdown(f"""
- **Noise** increases target variance and lowers **R²**; conformal intervals widen accordingly (when MAPIE active).
- **Trees/depth** control model capacity; over‑simple trees underfit (low R²), too deep risks variance.
- **Permutation FI** shows reliance on features; if ΔR²≈0 for a feature, model is insensitive to it.
- **Partial dependence** traces marginal effect; curvature implies interactions or nonlinearity.
""")
    interval_width = float(np.nanmean(np.nan_to_num(hi - lo))) if lo is not None and hi is not None else float("nan")
    log_event("landscape", {
        "n": n, "noise": float(noise), "trees": trees, "depth": depth,
        "r2": float(r2), "auc": float(auc), "mean_PI_width": interval_width
    })
