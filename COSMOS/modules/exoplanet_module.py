...
def panel():
    ...
    with c1:
        ...
        ax.set_xlabel("Trial Period"); ax.set_ylabel("SSE"); ax.set_title("Period scan: minima indicate candidate periods")
        st.pyplot(fig)
    with c2:
        phase = (time % bestP)/bestP
        fig2, ax2 = plt.subplots(figsize=(6,3))
        ax2.plot(phase, flux, ".", ms=2, alpha=0.6)
        ax2.set_xlabel("Phase"); ax2.set_ylabel("Flux"); ax2.set_title("Folded light curve at best period")
        st.pyplot(fig2)
...
Minima indicate periods where a box model explains the data well. A deeper transit and longer duty cycle produce a sharper minimum.  
• *Principle:* periodic change‑point detection; implicit matched‑filtering to a box template.  
• *Risk:* aliases if the grid is coarse or stellar variability mimics a box.

**2) Folded light curve**  
Data folded at **bestP={bestP:.4f}** should show phase‑aligned dips. The **duty‑cycle≈{fit['width']:.3f}** relates to transit duration and orbital geometry; the **depth≈{fit['depth']:.2e}** ∝ (R_p/R_★)^2 in the small‑planet limit.

**3) Residuals**  
After subtracting the box model, the residuals should look noise‑like. **SNR≈{snr:.2f}** (depth / σ_resid) is a quick detectability proxy. Structure here signals model mismatch (e.g., limb darkening, spot crossings).

**What this says about the universe**  
Photometric dips encode **planet size** (depth), **orbit period** (minima location), and **geometry** (duty cycle). With better models (Mandel–Agol + GP detrending), the same logic scales to survey‑grade inference.
""")
