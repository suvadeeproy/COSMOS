...
def _box_fit(time, flux, period):
    ...
    best = (np.inf, 0, 0); mu_out = np.median(flux)
    for w in widths:
        for phi0 in grid:
...
def panel():
    st.markdown("#### Exoplanets — BLS‑like search, box‑fit, residuals, SNR, math")
    ...
    if lk is not None and use_real:
        ...
        try:
            ...
            st.warning(f"Real-path failed: {e}. Falling back to synthetic."); lk = None
    ...
    with c1:
        ...
        ax.set_xlabel("Trial Period"); ax.set_ylabel("SSE"); ax.set_title("Period scan (box matched‑filter proxy)")
        st.pyplot(fig)
    with c2:
    ...
    ax3.plot(resid, "-", lw=0.7); ax3.set_title("Residuals after box model"); ax3.set_xlabel("sample"); ax3.set_ylabel("flux - model")
    st.pyplot(fig3)

    with st.expander("Math/physics notes (derivations & meaning)"):
        st.latex(r"\textbf{BLS objective: } \quad \min_{P,\,\phi,\,w,\,\delta}\; \sum_t \big(f_t - m_{P,\phi,w,\delta}(t)\big)^2")
        st.markdown("- Here \(m_{P,\phi,w,\delta}\) is the box template: out-of-transit mean \(\mu_{\rm out}\) and in-transit \(\mu_{\rm out}-\delta\).")
        st.latex(r"\textbf{Transit depth: } \quad \delta \approx (R_p/R_\star)^2\;\text{ (small-planet limit)}")
        st.latex(r"\textbf{Duty cycle: } \quad D \simeq \frac{T}{P} \approx \frac{R_\star}{\pi a} \quad (e\!=\!0) ")
        st.latex(r"\textbf{Kepler: } \quad P^2 = \frac{4\pi^2 a^3}{G M_\star}")
        st.latex(r"\textbf{SNR: } \quad \mathrm{SNR} = \frac{\delta}{\sigma_{\rm resid}}\)")
        st.markdown("Aliasing: if the period grid is coarse or noise is quasi-periodic, minima can recur at harmonic ratios.")
