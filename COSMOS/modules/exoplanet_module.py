...
def _bls_period(time, flux, minp, maxp, n=200):
...
def panel():
    st.markdown("#### Exoplanets — BLS-like period search, box‑fit, residuals, SNR")
    ...
    mu_out = fit['mu_out']
    phase = (time % bestP)/bestP
    in_mask = ((phase >= fit['phi0']) & (phase <= (fit['phi0']+fit['width']) % 1.0)) if fit['phi0']+fit['width']<=1 else ((phase>=fit['phi0']) | (phase <= fit['phi0']+fit['width']-1))
    model = np.where(in_mask, mu_out - fit['depth'], mu_out)
    resid = flux - model
    snr = fit['depth']/np.std(resid) if np.std(resid)>0 else 0.0
    ...
    # Live physics explainer & telemetry
    with st.expander("Physics discussion • What changed with your controls?"):
        st.markdown(f"""
- **Data source:** {data_source.upper()} — real uses SPOC pipeline (flattened); synthetic injects box‑dips into a noisy baseline.
- **Period search:** Coarse BLS analogue scans **P** to minimize in‑box/out‑of‑box SSE; your controls modify SNR and detectability.
- **Box fit:** We refine **phase** and **width**; **depth** is the median gap between out‑of‑transit and in‑transit levels.
- **Residuals:** White‑noise‑like residuals indicate a good fit; structure suggests stellar variability/systematics.
- **SNR:** \(\text{SNR}\approx\Delta/\sigma_r\). Increase depth or decrease jitter to raise SNR.
""")
    log_event("exoplanet", {
        "best_period": float(bestP), "depth": float(fit["depth"]), "width": float(fit["width"]),
        "snr": float(snr), "source": data_source
    })
