...
def solve_fd(kind: str, k: float, z_max: float = 8.0, N: int = 400, n_eigs: int = 6, z0: float = 8.0):
    """Finite-difference eigen-solve for Schrödinger-form Sturm–Liouville: -ψ'' + Vψ = m^2 ψ."""
    z_min = 1e-3
...
def panel():
    ...
    with c1:
        ...
        ax.set_xlabel("n"); ax.set_ylabel("m^2"); ax.legend()
        ax.set_title("Spectrum: FD vs WKB vs analytic trend")
        st.pyplot(fig)
    with c2:
        fig2, ax2 = plt.subplots(figsize=(4,3))
        for i in range(min(3, psi.shape[0])):
            ax2.plot(z, psi[i], label=f"ψ_{i}")
        ax2.set_xlabel("z"); ax2.set_ylabel("ψ(z)"); ax2.legend()
        ax2.set_title("Eigenfunctions (first three)")
        st.pyplot(fig2)
    with c3:
        fig3, ax3 = plt.subplots(figsize=(4,3))
        ax3.plot(z, V); ax3.set_xlabel("z"); ax3.set_ylabel("V(z)")
        ax3.set_title("Potential profile")
        st.pyplot(fig3)
    ...
    ax4.set_title("Convergence of m0^2")
    st.pyplot(fig4)

    with st.expander("Physics + math discussion (plot‑by‑plot)"):
        if kind=="soft-wall":
            st.markdown(f"""
**Equation (operator statement)**  
You are diagonalizing a differential operator:
\[
\mathcal{{L}}\psi \equiv -\frac{{d^2}}{{dz^2}}\psi(z) + V(z)\psi(z) = m^2\psi(z).
\]
In effective holographic models, \(m^2\) corresponds to mass‑squared levels; \(V(z)\) encodes confinement physics.

**Soft‑wall potential**  
\[
V(z)=k^2 z^2 + \frac{3}{4z^2}.
\]
The quadratic term produces (approx.) linear Regge trajectories; the \(1/z^2\) term fixes UV scaling.

---
**1) Spectrum plot**  
WKB says (schematically)
\[
\int_{{z_-}}^{{z_+}}\sqrt{{m^2-V(z)}}\,dz \approx \pi\left(n+\tfrac12\right).
\]
For this cartoon, analytic trend:
\[
m_n^2 \approx 4k(n+1).
\]
We estimate slope \(\partial m_n^2/\partial(n+1)\): **{slope:.3f}** vs **4k={4*k:.3f}**, rel. error **{rel_err:.2%}**.

*Physics reading:* near‑linearity means a single confinement scale dominates; deviations measure cutoff/discretization/boundary effects.

---
**2) Eigenfunctions plot**  
Node count grows with \(n\) (oscillation theorem). Localization and tails reflect \(V(z)\). Physically: higher excitations probe larger “IR” region (larger \(z\)).

---
**3) Potential plot**  
This is the “mechanism” plot: it tells you why the spectrum looks the way it does. Change the potential → change the spectrum.

---
**4) Convergence plot**  
Finite differences are an approximation \(\psi''\approx (\psi_{{i+1}}-2\psi_i+\psi_{{i-1}})/\Delta z^2\). As \(\Delta z\to 0\), eigenvalues should stabilize.
Numerical quality proxy: off‑diagonal orthonormality **{offdiag:.2e}** (smaller is better).

---
**Universe-level takeaway**  
“Spectra encode dynamics” is universal: hadrons, atoms, cavities, and black‑hole quasinormal modes all reduce (in some regime) to eigenproblems.
""")
        else:
            st.markdown(f"""
**Equation:** \( -\psi'' + V\psi = m^2\psi \). The hard wall at \(z_0={z0:.2f}\) imposes an IR boundary condition.  
Interpret the plots as: operator + boundary → spectrum; eigenfunctions diagnose BC/grid; convergence certifies numerics.
""")
