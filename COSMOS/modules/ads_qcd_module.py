...
def panel():
    st.markdown("#### AdS/QCD — FD vs WKB spectra, eigenfunctions, potential, convergence")
    cols = st.columns(5)
    with cols[0]:
    ...
    try:
        ...
        st.error(f"FD failed: {e}. Using a smaller grid.")
    ...
    with c1:
        ...
        ax.set_xlabel("n"); ax.set_ylabel("m²"); ax.legend(); ax.set_title("Spectrum")
        st.pyplot(fig)
    with c2:
    ...
    ax4.plot(sizes, vals0, 'o-'); ax4.set_xlabel("N grid"); ax4.set_ylabel("m₀²")
    ax4.set_title("Convergence of m₀²"); st.pyplot(fig4)

    with st.expander("Physics notes"):
        st.markdown(f"""
**Operator**  
\(L\psi = -\psi'' + V(z)\psi = m^2\psi\).  
Soft‑wall: \(V=k^2 z^2 + \tfrac{3}{4z^2}\). Hard‑wall: Dirichlet at \(z_0\).

**Semiclassics**  
WKB: \(\int\sqrt{m^2 - V}\,dz = \pi\,(n+\tfrac12)\).  
Soft‑wall gives \(m_n^2 \approx 4k(n+1)\); fitted slope {slope:.3f}, target {4*k:.3f}.

**Diagnostics**  
Orthonormality via \(\langle\psi_m,\psi_n\rangle\).  
Virial estimate: \(2T \approx \int \psi^2 z V'(z)\,dz\).  
Convergence with grid refinement tracks discretization error.
"""))
