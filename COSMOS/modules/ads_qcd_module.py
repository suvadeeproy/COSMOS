...
def potential(z, kind, k, z0):
    ...
    if kind == "soft-wall":
        ...
        V = 0.75/np.maximum(z**2, eps); V[z>z0] = 1e9; return V
    else:
        V = (k**2)*(z**2) + 0.75/np.maximum(z**2, eps); V[z>z0] = 1e9; return V

def solve_fd(kind: str, k: float, z_max: float = 8.0, N: int = 400, n_eigs: int = 6, z0: float = 8.0):
    z_min = 1e-3
    z = np.linspace(z_min, z_max, N); dz = z[1]-z[0]
...
def wkb_levels(kind, k, z0, nmax=6, zmax=12.0, Nz=4000):
    z = np.linspace(1e-4, zmax, Nz); V = potential(z, kind, k, z0)
    levels = []
    for n in range(nmax):
        target = np.pi*(n+0.5)
...
def panel():
    st.markdown("#### AdS/QCD — SL operator, WKB, virial, discretization order")
    cols = st.columns(5)
    with cols[0]:
    ...
    with st.expander("Math/physics notes (derivations & meaning)"):
        st.latex(r"\textbf{Sturm–Liouville form: }\; -\psi''(z) + V(z)\,\psi(z) = m^2\,\psi(z),\quad \psi(0)=\psi(z_{\max})=0")
        st.latex(r"\textbf{Soft‑wall potential: }\; V(z)=k^2 z^2 + \frac{3}{4 z^2}\;\Rightarrow\; m_n^2\approx 4k(n+1)")
        st.latex(r"\textbf{WKB quantization: }\; \int_{z_1}^{z_2} \sqrt{m^2 - V(z)}\,dz = \pi\,(n+\tfrac{1}{2})")
        st.latex(r"\textbf{Virial check (1D): }\; 2\langle T\rangle = \langle z\,V'(z)\rangle\)")
        st.markdown("Second‑order centered stencil ⇒ eigenvalue error \(\mathcal{O}(\Delta z^2)\) in smooth regions.")
