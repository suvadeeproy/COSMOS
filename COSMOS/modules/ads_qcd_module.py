...
def panel():
    ...
    with c2:
        ...
        for i in range(min(3, psi.shape[0])):
            ax2.plot(z, psi[i], label=f"psi_{i}")
        ax2.set_xlabel("z"); ax2.set_ylabel("psi(z)"); ax2.legend(); ax2.set_title("Eigenfunctions")
        st.pyplot(fig2)
    with c3:
    ...
    with st.expander("Physics discussion (live)"):
        if kind=="soft-wall":
            st.markdown(f"""
- Soft-wall predicts m_n^2 ~ 4 k (n+1). Fitted slope: **{slope:.3f}**, target 4k={4*k:.3f}, relative error **{rel_err:.3%}**.
- Off-diagonal orthonormality max ~ **{offdiag:.2e}** (should shrink with finer grids).
""")
        else:
            st.markdown(f"""
- WKB vs FD alignment reflects boundary effects from the hard wall at z0={z0:.2f}.
- Off-diagonal orthonormality max ~ **{offdiag:.2e}** (grid/refinement indicator).
""")
