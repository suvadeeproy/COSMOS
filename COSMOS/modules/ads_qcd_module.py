...
def wkb_levels(kind, k, z0, nmax=6, zmax=12.0, Nz=4000):
    # Quantization: ∫_{z1}^{z2} sqrt(m^2 - V(z)) dz = π(n+1/2)
    z = np.linspace(1e-4, zmax, Nz)
    V = potential(z, kind, k, z0)
    levels = []
    for n in range(nmax):
        target = np.pi*(n+0.5)
        # bracket search over m^2
...
def virial_check(z, psi, V):
    # <T> = <H> - <V>; check 2<T> ~ <z dV/dz> for soft-wall-ish regions (heuristic)
    dz = np.gradient(z)
    dpsi = np.gradient(psi, z, axis=1)
    T = 0.5*np.trapz((dpsi**2), z, axis=1)  # kinetic-ish (up to constants)
...
def panel():
    ...
    st.markdown("##### Orthonormality & virial checks")
    S = orthonormality(psi[:min(4, psi.shape[0])], z)
    T, Vexp, vir = virial_check(z, psi[:min(4, psi.shape[0])], V)
    st.write("Orthonormality matrix (first 4):")
    st.dataframe(S, use_container_width=True)
    st.write("Kinetic ⟨T⟩, Potential ⟨V⟩, and ⟨z ∂V/∂z⟩ (first 4 states):")
    st.dataframe(
        {"T": T[:min(4,len(T))], "V": Vexp[:min(4,len(Vexp))], "z dV/dz": vir[:min(4,len(vir))]},
